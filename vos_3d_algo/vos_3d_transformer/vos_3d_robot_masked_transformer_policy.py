...
class VOS3DRobotMaskedTransformerPolicy(nn.Module, metaclass=PolicyMeta):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self,
        ...
        wrist_kwargs = policy_cfg.wrist_encoder.network_kwargs
        wrist_kwargs.input_shape = shape_meta["all_shapes"]["eye_in_hand_depth"]
        wrist_kwargs.output_size = embed_size
        self.wrist_depth_encoder = ResnetEncoder(**wrist_kwargs)
    ...
    def spatial_encode(self, data):
        ...
        for name in self.pcd_encoders.keys():
            ...
            if self.training:
                ...
                # x1[:, :, -self.cfg.policy.pcd_encoder.network_kwargs.group_cfg.num_group:, ...] = 0.
                # x2[:, :, -self.cfg.policy.pcd_encoder.network_kwargs.group_cfg.num_group:, ...] = 0.     
                # x1 = rearrange(x1, "b t o g n d -> (b t o) g n d", b=B, t=T, o=O)
                # x2 = rearrange(x2, "b t o g n d -> (b t o) g n d", b=B, t=T, o=O).squeeze(-2)      
                ...
                # x1 = rearrange(x1, "(B T O) g D N -> (B T) O g D N", B=1, T=1)
                # x2 = rearrange(x2, "(B T O) D N -> (B T) O D N", B=1, T=1)
                # x1[:, -1, ...] = 0.
                # x2[:, -1, ...] = 0.
                # x1 = rearrange(x1, "(B) O g D N -> (B O) g D N")
                # x2 = rearrange(x2, "(B) O D N -> (B O) D N")
                x, _ = self.pcd_encoders[name]["encoder"].forward_mask(x1, x2)
                # x, _ = self.pcd_encoders[name]["encoder"](x.contiguous())
