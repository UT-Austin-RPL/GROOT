...
class VOS3DObjectTransformerPolicy(nn.Module, metaclass=PolicyMeta):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def spatial_encode(self, data):
        ...
        for name in self.pcd_encoders.keys():
            ...
            x = self.pcd_aug(x)
            x = x[:, :, :-1, ...]
            x = rearrange(x, 'B T O N D -> (B T O) N D', T=T, O=O-1, N=N)
            x = self.pcd_encoders[name]["encoder"](x)
            x = rearrange(x, '(B T O) D -> B T O D', B=B, T=T, O=O-1)
