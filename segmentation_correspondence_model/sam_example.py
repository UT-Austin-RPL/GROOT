...
parser.add_argument(
    "--input",
    type=str,
    default="segmentation_correspondence_model/target_images",
...
parser.add_argument(
    "--output",
    type=str,
    default="segmentation_correspondence_model/sam_results",
    help=(
...
parser.add_argument(
    "--checkpoint",
    type=str,
    default="third_party/sam_checkpoints/sam_vit_b_01ec64.pth",
...
def main(args: argparse.Namespace) -> None:
    ...
    print("saving masks to ", args.output)
    print("Loading checkpoints: ", args.checkpoint)
    print("Loading model type: ", args.model_type)
    ...
    for t in targets:
        demo_key = t.split("/")[-1].split(".")[0]
        folder_name = t.split("/")[-2]
        ...
        overall_mask = show_anns(masks)
        print(overall_mask.max(), overall_mask.shape)
        cv2.imwrite(os.path.join(output_dir, f"{demo_key}_vis.png"), (overall_mask * 255).astype(np.uint8))

        from PIL import Image
        def merge_segmentation_masks(arrays):
        ...
        def save_segmentation_mask(merged_mask, filepath):
        ...
        save_segmentation_mask(merged_mask, os.path.join(output_dir, f"{demo_key}_mask.png"))
